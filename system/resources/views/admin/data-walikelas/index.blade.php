    ...
    <main class="main users chart-page" id="skip-target" style="padding: 20px; display: flex; justify-content: center;">
        <div class="table-container" style="width: 100%; max-width: 1200px; background-color: #ffffff; border-radius: 12px; padding: 20px; box-shadow: 0px 6px 15px rgba(0, 0, 0, 0.1);">
            <div style="display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 10px; margin-bottom: 20px;">
                <!-- Button Tambah Guru -->
                <a href="{{ url('admin/data-walikelas/create') }}" 
                ...
                <!-- Search Form -->
                <form id="search-form" action="{{ url('admin/data-walikelas') }}" method="GET" 
                      style="display: flex; gap: 5px; max-width: 400px; width: 100%;">
                    <input type="text" id="search" name="search" value="{{ request()->get('search') }}" placeholder="Cari Walikelas..." 
             ...
             <div style="overflow-x: auto;">
                    ...
                    <tbody>
                        @foreach ($list_walikelas as $index => $walikelas)
                            <tr style="background-color: #f8f8f8; transition: background-color 0.3s; border-bottom: 1px solid #ddd;">
                                ...
                                <td style="padding: 12px;">{{ $walikelas->nip }}</td>
                                <td style="padding: 12px;">{{ $walikelas->nama }}</td>
                                <td style="padding: 12px;">{{ $walikelas->alamat }}</td>
                                <td style="padding: 12px;">{{ $walikelas->email }}</td>
                                <td style="padding: 12px; text-align: center;">
                                    <a href="{{ url('admin/data-walikelas/' . $walikelas->id) }}"
                                    ...
                                    <!-- Edit button -->
                                    <a href="{{ url('admin/data-walikelas/' . $walikelas->id . '/edit') }}"  
                                    ...
                                    <!-- Delete button -->
                                    <form action="{{ url('admin/data-walikelas/' . $walikelas->id) }}" method="POST" id="deleteForm{{ $walikelas->id }}" style="display: inline;">
                                        @csrf
                                        <input type="hidden" name="_method" value="DELETE">
                                        <button type="button" onclick="showConfirmModal({{ $walikelas->id }})" 
                                    ...
                                    <!-- Modal Konfirmasi -->
                                    <div id="confirmModal{{ $walikelas->id }}" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.5); align-items: center; justify-content: center;">
                                        <div style="background-color: white; padding: 15px; border-radius: 8px; width: 300px; text-align: center;">
                                            ...
                                            <button id="confirmDeleteBtn{{ $walikelas->id }}" style="background-color: #e74c3c; color: white; padding: 5px 10px; border: none; border-radius: 5px; cursor: pointer; font-weight: bold; font-size: 14px; margin-top: 10px;">
                                                Hapus
                                            </button>
                                            <button onclick="hideConfirmModal({{ $walikelas->id }})" style="background-color: #ccc; color: black; padding: 5px 10px; border: none; border-radius: 5px; cursor: pointer; font-weight: bold; font-size: 14px; margin-top: 10px;">
                                                Batal
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <script>
                                        function showConfirmModal(walikelasId) {
                                            // Tampilkan modal
                                            document.getElementById('confirmModal' + walikelasId).style.display = 'flex';
                                    
                                            // Set tindakan konfirmasi
                                            document.getElementById('confirmDeleteBtn' + walikelasId).onclick = function() {
                                                document.getElementById('deleteForm' + walikelasId).submit();
                                            };
                                        }
                ...
                <div style="margin-top: 20px; display: flex; justify-content: center;">
                    <ul style="display: flex; list-style: none; padding: 0; gap: 5px;">
                        <!-- Tombol Sebelumnya -->
                        <li>
                            <a href="{{ $list_walikelas->previousPageUrl() }}" style="padding: 4px 8px; border-radius: 5px; {{ $list_walikelas->onFirstPage() ? 'background-color: #ddd; color: #999; cursor: not-allowed;' : 'background-color: #4fb7e0; color: white;' }}">
                                &laquo; Sebelumnya
                            </a>
                        </li>
            
                        <!-- Nomor Halaman -->
                        @foreach ($list_walikelas->getUrlRange(1, $list_walikelas->lastPage()) as $page => $url)
                            <li>
                                <a href="{{ $url }}" style="padding: 4px 8px; border-radius: 5px; {{ $page == $list_walikelas->currentPage() ? 'background-color: #4fb7e0; color: white;' : 'background-color: #e0e0e0; color: #333;' }}">
                                    {{ $page }}
                                </a>
                            </li>
                        @endforeach
                        ...
                        <!-- Tombol Berikutnya -->
                        <li>
                            <a href="{{ $list_walikelas->nextPageUrl() }}" style="padding: 4px 8px; border-radius: 5px; {{ $list_walikelas->hasMorePages() ? 'background-color: #4fb7e0; color: white;' : 'background-color: #ddd; color: #999; cursor: not-allowed;' }}">
                                Berikutnya &raquo;
                            </a>
                        </li>
                    </ul>
