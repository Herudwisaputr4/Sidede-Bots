    ...
    <div class="nav_deg" style="margin: 10px;">
        <div style="display: flex; justify-content: center; align-items: center; width: 100%; height: 50px; background-color: #d12525; box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.15); border-radius: 8px;">
            <h1 style="margin: 0; padding: 0 10px; text-align: center; font-size: 20px; color: white;">
                Data Peserta Didik Sekolah Dasar
            </h1>
        </div>
    </div>
    ...
    <main class="main users chart-page" id="skip-target" style="padding: 20px; display: flex; justify-content: center;">
        <div class="table-container" style="width: 100%; max-width: 1200px; background-color: #ffffff; border-radius: 12px; padding: 20px; box-shadow: 0px 6px 15px rgba(0, 0, 0, 0.1);">
            <div style="display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 10px; margin-bottom: 20px;">
                <!-- Button Tambah Siswa -->
                <a href="{{ url('admin/data-siswa/create') }}" 
                ...
                <!-- Search Form -->
                <form id="search-form" action="{{ url('admin/data-siswa') }}" method="GET" 
                      style="display: flex; gap: 5px; max-width: 400px; width: 100%;">
                    <input type="text" id="search" name="search" value="{{ request()->get('search') }}" placeholder="Cari Peserta Didik..." 
            ...
            <div style="overflow-x: auto;">
                <table style="width: 100%; border-collapse: collapse;">
                    ...
                    <tr style="background-color: #747b7e; color: white;">
                        ...
                        <th style="padding: 12px; text-align: left; font-weight: bold;">Telepon</th>
                        <th style="padding: 12px; text-align: left; font-weight: bold;">Kelas</th>
                        <th style="padding: 12px; text-align: left; font-weight: bold;">Wali Kelas</th>
                        <th style="padding: 12px; text-align: center; font-weight: bold; border-radius: 0 8px 8px 0;">Aksi</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach ($list_siswa as $index => $siswa)
                        <tr style="background-color: #f8f8f8; transition: background-color 0.3s; border-bottom: 1px solid #ddd;">
                            ...
                            <td style="padding: 12px;">{{ $siswa->telepon }}</td>
                            <td style="padding: 12px;">{{ $siswa->kelas }}</td>
                            <td style="padding: 12px;">{{ $siswa->wali_kelas }}</td>

                            <td style="padding: 12px; text-align: center;">
                                <a href="{{ url('admin/data-siswa/' . $siswa->id) }}"
                                ...
                                <!-- Edit button -->
                                <a href="{{ url('admin/data-siswa/' . $siswa->id . '/edit') }}"  
                                ...
                                <!-- Delete button -->
                                <form action="{{ url('admin/data-siswa/' . $siswa->id) }}" method="POST" id="deleteForm{{ $siswa->id }}" style="display: inline;">
                                    @csrf
                                ...
                                <!-- Modal Konfirmasi -->
                                <div id="confirmModal{{ $siswa->id }}" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,0.5); align-items: center; justify-content: center;">
                                    <div style="background-color: white; padding: 15px; border-radius: 8px; width: 300px; text-align: center;">
                                        ...
                                        <button id="confirmDeleteBtn{{ $siswa->id }}" style="background-color: #e74c3c; color: white; padding: 5px 10px; border: none; border-radius: 5px; cursor: pointer; font-weight: bold; font-size: 14px; margin-top: 10px;">
                                            Hapus
                                        </button>
                                        <button onclick="hideConfirmModal({{ $siswa->id }})" style="background-color: #ccc; color: black; padding: 5px 10px; border: none; border-radius: 5px; cursor: pointer; font-weight: bold; font-size: 14px; margin-top: 10px;">
                                            Batal
                                        </button>
                                    </div>
                                </div>
            ...
            <div style="margin-top: 20px; display: flex; justify-content: center;">
                <ul style="display: flex; list-style: none; padding: 0; gap: 5px;">
                    <!-- Tombol Sebelumnya -->
                    <li>
                        <a href="{{ $list_siswa->previousPageUrl() }}" style="padding: 4px 8px; border-radius: 5px; {{ $list_siswa->onFirstPage() ? 'background-color: #ddd; color: #999; cursor: not-allowed;' : 'background-color: #4fb7e0; color: white;' }}">
                            &laquo; Sebelumnya
                        </a>
                    </li>
        
                    <!-- Nomor Halaman -->
                    @foreach ($list_siswa->getUrlRange(1, $list_siswa->lastPage()) as $page => $url)
                        <li>
                            <a href="{{ $url }}" style="padding: 4px 8px; border-radius: 5px; {{ $page == $list_siswa->currentPage() ? 'background-color: #4fb7e0; color: white;' : 'background-color: #e0e0e0; color: #333;' }}">
                                {{ $page }}
                            </a>
                        </li>
                    @endforeach
                    ...
                    <!-- Tombol Berikutnya -->
                    <li>
                        <a href="{{ $list_siswa->nextPageUrl() }}" style="padding: 4px 8px; border-radius: 5px; {{ $list_siswa->hasMorePages() ? 'background-color: #4fb7e0; color: white;' : 'background-color: #ddd; color: #999; cursor: not-allowed;' }}">
                            Berikutnya &raquo;
                        </a>
                    </li>
                </ul>
